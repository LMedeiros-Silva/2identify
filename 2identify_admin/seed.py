...
USUARIO_ADMIN = os.getenv("ADMIN_SEED_USERNAME", "admin").strip() or "admin"


def obter_senha_admin() -> str:
    """Lê uma senha futura sem armazená-la no código ou exibi-la no terminal."""

    senha_ambiente = os.getenv("ADMIN_SEED_PASSWORD")
    if senha_ambiente is not None:
        senha = senha_ambiente
    else:
        senha = getpass.getpass("Senha inicial do administrador: ")
        confirmacao = getpass.getpass("Confirme a senha inicial: ")
        if senha != confirmacao:
            raise RuntimeError("As senhas informadas não coincidem.")

    if len(senha) < 12:
        raise RuntimeError("A senha inicial deve ter pelo menos 12 caracteres.")
    if len(senha.encode("utf-8")) > 72:
        raise RuntimeError("A senha inicial excede o limite seguro do bcrypt.")
    return senha
...
def criar_usuario_admin(session, senha_admin: str | None = None):
    """
    ...
    senha_hash = gerar_hash_senha(
        senha_admin if senha_admin is not None else obter_senha_admin()
    )
...
def main():
    ...
    try:
        ...
        print("Senha: configurada de forma segura e não exibida.")
        print()
        ...
        print("=" * 60)
        print()
        print("Consulte o erro retornado pelo processo para diagnóstico.")
